...
module.exports = {
    ...
    configureVorhangZubehoer: function configureVorhangZubehoer(test) {
        ...
        total_mwst = total_mwst.replace('.', ',').replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.') //needed for separator 1.000
            ...
            checkOutPage.checkFinalPrices(test.system, test.rabatt_code, rabatt_betrag, sieSparen_mwst, versandkosten, streichPreis_mwst, streichPreisTotal_mwst, redPreis_mwst, redPreisTotal_mwst, total_mwst)
...
function setFarbe(value) {
    if (typeof value != "undefined") {
        cy.get('#options-config_gardinenstangen_farbe > #options-list-color').children().contains(value).click()
    }
}

function setBreite(value) {
    if (typeof value != "undefined") {
        cy.get('#options-config_gardinenstangen_breite > #options-list-color').children().contains(value).click()
    }
}

function checkSpecialPrice(price) {
    cy.get('.price-container > .special-price').should("contain", price)
}

function checkOriginalPrice(original_price) {
    cy.get('.price-container > .old-price > .price.high-price').should("contain", original_price)
}
