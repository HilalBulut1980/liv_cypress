...
Cypress.Commands.add('paypalFlow', (email, password) => {
    ...
    cy.get('#agreement-1').uncheck({ force: true })
    cy.get('#agreement-2').uncheck({ force: true })
    cy.wait(1000)
        ...
        .then($body => {
            if ($body.find('#pp-simulator').length) {  //if giropay-form exists
                ...
                if (canceled_payment == true) {
                    ...
                    // billing address --> clcik buttun 'Weiter'
                    cy.get('button[title="Weiter"]:visible').click()
                    ...
                    cy.get('dt[class="ppp bankpayment"]').click().wait(1000)
                    cy.get('dd[class="ppp bankpayment ppp-selected"]')  //only an additional check of the right selection
...
Cypress.Commands.add('sepaFlow', () => {
    ...
    // Click on the PayPal button inside PayPal's iframe
    cy.get('#checkout-review-submit iframe.component-frame').iframe().find('div[data-funding-source="sepa"]').click()
...
Cypress.Commands.add('giropayFlow', (failed_payment, canceled_payment) => {
    ...
    cy.get('body')
        .then($body => {
            if ($body.find('#pp-simulator').length) {  //if giropay-form exists
                cy.log('new window Giropay')

                if (canceled_payment == true) {

                    cy.log('GIROPAY TO BE CANCELED')
                    cy.get('button').contains('Test Canceled Payment').click()
...
Cypress.Commands.add('creditcardFlow', () => {
    ...
    cy.get('#checkout-review-submit iframe.component-frame').iframe().find('iframe.zoid-visible').then(($iframe) => {
        ...
        // cy.wrap($iframe).iframe().find('span').contains('Sie zahlen insgesamt:').next().should('contain', '796,38 EUR')
