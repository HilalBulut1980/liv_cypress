describe('Test: check if selected DF genormt is available with comfort/non-comfort', () => {
    ...
    it('DF20 genormt - check comfort / non-comfort with Fakro FYP-V 94 / 180 | Holz', function () {
        ...
        // set Typ
        cy.get('.dfselect_wrapper > :nth-child(3) select').select('21 A | Holz') 
        // --> 609 x 390 non-comfort PG4
        ...
        // check prices
        cy.get('.old-price').should('contain', '187,00')
        cy.get('.special-price').should('contain', '84,15') //-55% 
        ...
        // Streichpreis einzel 
        cy.get('span[class="cart-price"] > div > span[style="text-decoration: line-through;"]').should('contain', '187,00')
        // red. Preis einzel
        cy.get('span[class="cart-price"] > div > .price').should('contain', '84,15')
    })
})