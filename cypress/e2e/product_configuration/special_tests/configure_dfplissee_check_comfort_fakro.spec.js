describe('Test: check if selected DF genormt is available with comfort/non-comfort', () => {
    ...
    it('DF20 genormt - check comfort / non-comfort with Fakro FYP-V 94 / 180 | Holz', function () {
        ...
        // set Hersteller
        cy.get('.dfselect_wrapper > :nth-child(1) select').select('Fakro')

        // set Produkt
        cy.get('.dfselect_wrapper > :nth-child(2) select').select('FYP-V')

        // set Typ
        cy.get('.dfselect_wrapper > :nth-child(3) select').select('94 / 180 | Holz') 
        ...
        // check prices
        cy.get('.old-price').should('contain', '619,00')
        cy.get('.special-price').should('contain', '278,55') //-55% 
        ...
        // Streichpreis einzel 
        cy.get('span[class="cart-price"] > div > span[style="text-decoration: line-through;"]').should('contain', '619,00')
        // red. Preis einzel
        cy.get('span[class="cart-price"] > div > .price').should('contain', '278,55')
    })
})