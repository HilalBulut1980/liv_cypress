describe('Test: check if selected DF genormt is available with comfort/non-comfort', () => {
    ...
    it('DF20 genormt - check comfort / non-comfort with Fakro FYP-V 94 / 180 | Holz', function () {
        ...
        cy.get('.type-selector-left > ul').children().contains('DF20 Comfort').click({ force: true })

        // following product is only available with non-comfort

        // set Hersteller
        cy.get('.dfselect_wrapper > :nth-child(1) select').select('Roto')

        // set Produkt
        cy.get('.dfselect_wrapper > :nth-child(2) select').select('SA')

        // set Typ
        cy.get('.dfselect_wrapper > :nth-child(3) select').select('11 / 16 | Kunststoff / Holz') 
        ...
        cy.get('.type-selector-left > ul').children().contains('DF20 Comfort').parent('li').should('not.have.class', 'selected')
        cy.get('.type-selector-left > ul').children().contains('DF20').parent('li').should('have.class', 'selected')

        // check prices
        cy.get('.old-price').should('contain', '493,00')
        cy.get('.special-price').should('contain', '221,85') //-55% 
        ...
        // red. Preis einzel
        cy.get('span[class="cart-price"] > div > .price').should('contain', '221,85')
    })
})